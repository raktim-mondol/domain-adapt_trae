...
from src.utils.training import train_model_ssda
...
class DummyUnlabeledDataset(Dataset):
    def __init__(self, n=20, num_patches=12, size=64):
        self.n = n
        self.num_patches = num_patches
        self.size = size
    ...
    def __getitem__(self, idx):
        bag = torch.randn(self.num_patches, 3, self.size, self.size)
        label = 0
        pile_id = f'pile_u_{idx}'
        image_path = f'img_u_{idx}.png'
        return bag, label, pile_id, image_path


def main():
    Config.BEST_MODEL_PATH = 'best_dummy_ssda.pth'
    ...
    tgt_l_ds = DummyLabeledDataset(n=10)
    tgt_u_ds = DummyUnlabeledDataset(n=20)
    val_ds = DummyLabeledDataset(n=10)
    src_loader = DataLoader(src_ds, batch_size=2, shuffle=True)
    tgt_l_loader = DataLoader(tgt_l_ds, batch_size=2, shuffle=True)
    tgt_u_loader = DataLoader(tgt_u_ds, batch_size=2, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=2, shuffle=False)
    train_losses, val_accuracies, val_f1_scores = train_model_ssda(
        model=model,
        train_loader_source=src_loader,
        train_loader_target_labeled=tgt_l_loader,
        train_loader_target_unlabeled=tgt_u_loader,
